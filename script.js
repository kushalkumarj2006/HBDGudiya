...
function createGlowingFlowers() {
  ...
  for (let i = 0; i < 80; i++) {
    ...
    Flower.style.fontSize = (Math.random() * 40 + 25) + 'px';
...
function createFlowerExplosion() {
  ...
  for (let i = 0; i < 100; i++) {
    const Flower = document.createElement('div');
    Flower.classList.add('exploding-Flower');
    Flower.innerHTML = '🌺';
    Flower.style.left = '50%';
    Flower.style.top = '50%';
    Flower.style.fontSize = (Math.random() * 30 + 20) + 'px';
    Flower.style.color = `hsl(${Math.random() * 60 + 320}, 90%, 60%)`;
    Flower.style.animation = `explode ${Math.random() * 2 + 1}s forwards`;
    Flower.style.opacity = '0';
    
    setTimeout(() => {
      Flower.style.opacity = '1';
      Flower.style.transform = `translate(${Math.random() * 200 - 100}px, ${Math.random() * 200 - 100}px)`;
    }, 10);
  ...
  setTimeout(() => {
    ...
    const countdownInterval = setInterval(() => {
      if (count > 0) {
        ...
        countdownElement.innerHTML = "Happy Birthday<br>Gudiya ❤️";
